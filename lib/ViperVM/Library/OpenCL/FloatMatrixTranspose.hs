module ViperVM.Library.OpenCL.FloatMatrixTranspose where
...
kernel = do
   prog <- program
   initKernel prog "floatMatrixTranspose" [] config

-- | Configure kernel execution
config :: [CLKernelParameter] -> KernelConfiguration
config pms = KernelConfiguration gDim lDim pms
   where
      [CLUIntParam width, CLUIntParam height,_,_,_,_,_,_] = pms
